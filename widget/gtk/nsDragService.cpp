...
static void OnSourceGrabEventAfter(GtkWidget* widget, GdkEvent* event,
  ...
  if (event->type == GDK_MOTION_NOTIFY) {
    ...
    // the eDragEnd event.
    GdkWindow* gdkWindow = gtk_widget_get_window(sGrabWidget);
    nsWindow* win = gdkWindow ? nsWindow::GetWindow(gdkWindow) : nullptr;
    nsCOMPtr<nsIDragSession> session = win ? win->GetDragSession() : nullptr;
    if (session) {
...
nsIDragSession* nsDragService::StartDragSession(nsIWidget* aWidget) {
  LOGDRAGSERVICE("nsDragService::StartDragSession");
  return nsBaseDragService::StartDragSession(aWidget);
}
...
nsresult nsDragSession::EndDragSessionImpl(nsIWidget* aWidget, bool aDoneDrag,
                                           uint32_t aKeyModifiers) {
  LOGDRAGSERVICE("nsDragSession::EndDragSession(%p) %d",
  ...
  // if aWidget is null then this is the eDragTaskSourceEnd message sent
  // to the source widget of the drag.  We can get the widget from
  // mSourceDocument.
  if (!aWidget && mSourceDocument) {
    RefPtr<nsIWidget> widget =
        mozilla::widget::WidgetUtils::DocumentToWidget(mSourceDocument);
    aWidget = widget.get();
  }
  ...
  return nsBaseDragSession::EndDragSessionImpl(aWidget, aDoneDrag,
                                               aKeyModifiers);
}
...
void nsDragSession::SourceEndDragSession(GdkDragContext* aContext,
  ...
  if (mEndDragPoint.x < 0) {
    ...
    if (mSourceWindow) {
      if (auto* session = mSourceWindow->GetDragSession()) {
...
gboolean nsDragSession::ScheduleDropEvent(nsWindow* aWindow,
  ...
  if (auto* session = aWindow->GetDragSession()) {
...
gboolean nsDragSession::RunScheduledTask() {
  ...
  if (mTargetWindow && mTargetWindow != mPendingWindow) {
    ...
    if (!mSourceNode) {
      ...
      // back into this app).
      RefPtr<nsIDragSession> session = mTargetWindow->GetDragSession();
      if (session) {
        RefPtr<nsIWidget> win = mTargetWindow;
        session->EndDragSession(win, false, GetCurrentModifiers());
      }
    }
  }
  ...
  if (task == eDragTaskLeave || task == eDragTaskSourceEnd) {
    ...
    if (task == eDragTaskSourceEnd) {
      ...
      if (session) {
        RefPtr<nsIWidget> win = mTargetWindow;
        session->EndDragSession(win, true, GetCurrentModifiers());
      }
    }
...
void nsDragSession::DispatchMotionEvents() {
  if (mSourceWindow) {
    if (RefPtr<nsIDragSession> session = mSourceWindow->GetDragSession()) {
