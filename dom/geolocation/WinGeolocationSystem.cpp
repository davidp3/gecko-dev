...
GetWifiControlAppCapability() {
  ...
  hr = appCapabilityStatics->CreateWithProcessIdForUser(
      user, HStringReference(L"wifiControl").Get(), ::GetCurrentProcessId(),
...
already_AddRefed<OpenSettingsPromise::Private> OpenWindowsLocationSettings() {
  ...
  ComPtr<ILauncherStatics> launcherStatics =
      CreateFromActivationFactory<ILauncherStatics>(
  ...
  ComPtr<IUriRuntimeClassFactory> uriFactory =
      CreateFromActivationFactory<IUriRuntimeClassFactory>(
  ...
  HRESULT hr = uriFactory->CreateUri(
      HStringReference(L"ms-settings:privacy-location").Get(),
      getter_AddRefs(uri));
  if (FAILED(hr)) {
    return nullptr;
  }
...
Maybe<ABI::Windows::Security::Authorization::AppCapabilityAccess::
...
already_AddRefed<OpenSettingsPromise::Private> PresentSystemSettings() {
  RefPtr<OpenSettingsPromise::Private> openPromise =
  ...
  // granted (and rejects when the user presses cancel in the modal dialog).
  ...
  openPromise->Then(
      ...
      [locationListener, retPromise](bool aWasOpened) {
        if (locationListener->IsStopped()) {
          // We are already canceled.
          return false;
        }
        if (!aWasOpened) {
          return false;
        ...
        appCapability->add_AccessChanged(
            Callback<AccessChangedHandler>(
                [locationListener, retPromise](
                    IAppCapability*, IAppCapabilityAccessChangedEventArgs*) {
                  if (locationListener->IsStopped()) {
                    // We are already canceled.  This watchpoint is ending.
                    return S_OK;
                  }
  ...
  retPromise->Then(
      GetCurrentSerialEventTarget(), __func__,
      [locationListener]() {
        // We got system permission.  Nothing to do here.
        MOZ_ASSERT(locationListener->IsStopped());
      },
      [locationListener]() {
